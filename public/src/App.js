...
function App() {
  ...
  const handleStartVoting = async () => {
    try {
      ...
      setVotingOpen(true);
      showToast("Голосование успешно начато!");
    } catch (error) {
      console.error("StartVoting error:", error);
      showToast("Ошибка при начал голосования", 'error');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleRemoveCandidate = async (index) => {
    try {
      ...
      addTransaction(
        'Удаление кандидата',
        tx.hash,
        `Удален кандидат: ${candidates[index].name}`
      );
      
      await loadCandidates(contract);
      showToast("Кандидат успешно удален!");
    } catch (error) {
      console.error("Remove candidate error:", error);
      if (error.message.includes("voting is open")) {
        showToast("Нельзя удалять кандидатов во время голосования", 'error');
      } else {
        showToast("Ошибка при удалении кандидата", 'error');
      }
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="wallet-section">
        {currentAccount ? (
          <div className="wallet-info">
            <span>Подключенный адрес:</span>
            <span className="wallet-address">
              {currentAccount}
              {currentAccount.toLowerCase() === owner.toLowerCase() && (
                <span className="admin-badge">Администратор контракта</span>
              )}
            </span>
          </div>
        ) : (
          <>
            <p>Пожалуйста подключите кошелек для взаимодействия с DApp.</p>
        ...
        <div className="add-candidate-section">
          <div className="input-row">
            <div className="name-input">
              <input
                ...
                maxLength={50}
              />
              <small>{newCandidate.length}/50 символов</small>
            </div>
            ...
            <div className="file-input">
              ...
              <span className="file-status">
                {selectedFile ? "✓ Файл выбран" : "Файл не выбран"}
              </span>
            </div>
          </div>
          ...
          <div className="program-input">
            <textarea
              ...
              placeholder="Введите предвыборную программу"
              maxLength={1000}
            />
            <small>{candidateProgram.length}/1000 символов</small>
          </div>
          ...
          <div className="button-container">
            ...
            >
              {isLoading ? "Добавление..." : "Добавить кандидата"}
            </button>
          </div>
        </div>
      )}
      ...
      {currentAccount && owner && currentAccount.toLowerCase() === owner.toLowerCase() && (
        <div className="voting-controls">
          {!votingOpen && (
            ...
            >
              {isLoading ? "Начало голосования..." : "Начать голосование"}
            </button>
          )}
          {votingOpen && (
            <button 
              ...
              disabled={isLoading}
            >
              {isLoading ? "Завершение голосования..." : "Завершить голосование"}
            </button>
          )}
        </div>
      )}
      ...
      {votingEnded && votingEndedAt && (
        <>
          <div className="voting-info">
            <p>Голосование завершено: {formatEndTime(votingEndedAt)}</p>
          </div>
          {winner.name && (
            <div className="winner-section">
              <h3>🎉 ПОБЕДИТЕЛЬ ГОЛОСОВАНИЯ 🎉</h3>
              <div className="winner-content">
                ...
                <div className="winner-info">
                  ...
                  <div className="winner-votes">{winner.votes} голосов</div>
                </div>
              </div>
            </div>
          )}
        ...
        <ul className="candidates-list">
          {candidates.map((candidate, index) => (
            <li key={index} className="candidate-item">
                ...
                <div className="candidate-details">
                  ...
                  <div className="candidate-program-container">
                    ...
                    {candidate.program.length > 300 && (
                      ...
                      >
                        {expandedPrograms[index] ? 'Свернуть' : 'Подробнее'}
                      </button>
                    )}
                  </div>
                  <span className="vote-count">{candidate.voteCount || '0'} голосов</span>
                  {currentAccount && 
                   owner && 
                   currentAccount.toLowerCase() === owner.toLowerCase() && 
                   !votingOpen && (
              ...
              {votingOpen && (
                <button onClick={() => handleVote(index)}>Голосовать</button>
              )}
            </li>
          ))}
        </ul>
